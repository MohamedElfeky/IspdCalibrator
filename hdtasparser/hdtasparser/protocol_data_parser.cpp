...
namespace hdtas
{
    ...
    po_tcp_cfg_read_req::po_tcp_cfg_read_req() :
            ...
            des_mr_id_(0), pair_id_(0), status(0)
    {
    ...
    po_tcp_cfg_read_rpd::po_tcp_cfg_read_rpd() :
            ...
            pair_id_(0),des_mr_id_(0),ecode(0),
            static_ip_(0),static_netmask_(0),static_gateway_(0),static_dns_(0),
            dhcp_ip_(0),dhcp_netmask_(0),dhcp_gateway_(0), dhcp_dns_(0),
            tcp_port_(0),udp_port_(0),version_(0),
            network_type_(po_network_type::pnt_unknown), netmask_select_(po_network_select::pns_unknown),
            wifi_mode_(po_wifi_mode::pwm_unknown), dhcp_mode_(po_dhcp_mode::pdm_unknown)
    {
    }
    ...
    int po_tcp_cfg_read_rpd::deserialize_in( const unsigned char* buffer, size_t len, size_t& off )
    {
        ...
        // 后面多出来的一个字节指代域名字段，协议中肯定要包含要包含域名字段，
        // 而域名字段不固定长度（<=255），但是肯定要大于0，所以在此保守的估计了长度不能小于1。
        if ( DEFAULT_TCP_CFG_RD_RPD_WITH_ERR_LEN != len && len < DEFAULT_TCP_CFG_RD_RPD_LEN+1 )
        {
            this->strerr = "the buffer(";
            this->strerr += utility::uint64_2_str(len);
        ...
        // 问题：当向主基站读取未开机基站的配置信息时，主机站发现错误自动重启。
        // 原因：向未开机基站发送请求，主机站返回了未找到目的基站的错误信息，但是只返回了前10个必要的字节，
        //       并没有返回整个数据结构，这就导致了在解析时出现了误判，误认为不识别的协议，而服务器强行关闭了连接。
        // 说明：已经同协议提供方商议了这种模式，可以出现这种由于发生错误，只返回必要数据，不返回完整数据结构的模式，
        //       此处则按照协议解析正确来处理。
        if ( DEFAULT_TCP_CFG_RD_RPD_WITH_ERR_LEN == len )
        {
            this->static_ip_ = this->static_netmask_ = this->static_gateway_ = this->static_dns_ = 0;
            this->mac_addr_.clear();
            this->dhcp_ip_ = this->dhcp_netmask_ = this->dhcp_gateway_ = this->dhcp_dns_ = 0;
            this->tcp_port_ = this->udp_port_ = 0;
            this->version_ = 0;
            this->network_type_ = po_network_type::pnt_unknown;
            this->wifi_mode_ = po_wifi_mode::pwm_unknown;
            this->dhcp_mode_ = po_dhcp_mode::pdm_open;
            this->domain_.clear();
            return 0;
        }
        ...
        // 问题：服务返回终端的MAC地址总是少一位字符，理论上MAC地址长12个字符，但却返回了前11个字符。
        // 原因：MAC地址长度位12，this->mac_addr_应该resize（13）而不是12。
        // 日期：2017-10-18 王海洋
        this->mac_addr_.resize( LEN_MAX_MACADDR_STRING+1, '0' );
        unsigned char mac[6] = { buffer[off++], buffer[off++], buffer[off++],
                                 buffer[off++], buffer[off++], buffer[off++] };
        SNPRINTF( &(this->mac_addr_[0]), LEN_MAX_MACADDR_STRING+1, "%02X%02X%02X%02X%02X%02X",
        ...
        // 原协议数据字节流中已经包含了域名服务器字符串结束符（"\0"），
        // 所以这里不需要多加入一个字节，实际上及时这样，
        // 也有一个std::string本身resize()时多余的字符串结束符（"\0"）。
        // this->domain_.resize( datasize+1 );
        this->domain_.resize( datasize );
    ...
    po_tcp_cfg_write_req::po_tcp_cfg_write_req() :
            ...
            status_(0),pair_id_(0),des_mr_id_(0),
            static_ip_(0),static_netmask_(0),static_gateway_(0),static_dns_(0),
            tcp_port_(0),udp_port_(0),
            netmask_select_(po_network_select::pns_auto)
    {
    }
    ...
    {
        // 问题：更新终端[服务器域名]字段值后，终端中的[服务器域名]有时会显示多余的字符串：
        //       如写入[服务器域名]：[mr.misbio.com]，结果终端中显示的[服务器域名]：[mr.misbio.com1.144]。
        // 原因：协议中应该包括[服务器域名]字段的字符串结束符"\0"，此处漏掉了该结束符"\0"，顾在此增加该结束符。
        if ( len < DEFAULT_TCP_CFG_WT_REQ_LEN+this->domain_.size()+1)
        {
    ...
    po_tcp_cfg_write_rpd::po_tcp_cfg_write_rpd() :
            ...
            des_mr_id_(0),pair_id_(0),ecode(0)
    {
    ...
    po_tcp_ctl_req::po_tcp_ctl_req() :
            ...
            status(0),cmd(0),pair_id_(0),
             data(NULL),data_cap_(0),data_len_(0)
    {
        this->data_len_ = 0;
        this->data_cap_ = 512;
    ...
    po_tcp_ctl_rpd::po_tcp_ctl_rpd() :
            ...
            pb_ecode_(0),ecode_(0),cmd_(0),addr_(0),
            pair_id_(0),mr_report_interval_(0),
            mr_id_(0),mr_update_id_(0),mr_update_msg_id_(0)
    {}
    ...
    int po_udp_data_resp2::deserialize_in( const unsigned char* buffer, size_t len, size_t& off )
    {
        ...
        for ( unsigned int i = 0; i < this->ispd_cnt; ++i )
        {
            ...
            if ( 15 == seni_len )
            {
                ...
                // 问题：超级版硬件方面修改了协议，原来使用两个字节表示心率，现在使用低8位表示心率。
                // 原因：协议变更。
                // 日期：2017-10-13 王海洋
                //ids.heart_rate = buffer[off++];
                //tempu16 = buffer[off++];
                tempu16 = 0;
                off += utility::deserialize_int( buffer+off, tempu16 );
