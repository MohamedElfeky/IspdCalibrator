...
namespace hdtas
{
                                     ...
                                     struct sockaddr_storage& addr )
    {
        ...
        {
            g_standard_error = false;
            const char* err = gai_strerror( rtn );
...
#ifdef HDTAS_WINDOWS
        const char* straddr = (const char*)InetNtop( addr.ss_family, inaddr, (PSTR)addrbuf, sizeof(addrbuf) );
#else
    ...
    const char* utility::err_str( int e )
    {
        switch ( e )
        {
            case H_ERR_SYSTEM:
            {
                ...
                NULL, e, 0, (LPSTR)g_err_buf, HDTAS_ERR_BUF_CAP, NULL);
                //return g_err_buf;
#else
                if ( !g_standard_error )
                {
    ...
    bool parser_mysql_info( const std::string& info, database_info& db_info )
    {
        ...
        if ( std::string::npos == sindex )
            return false;
        db_info.password = info.substr( findex+1, sindex-findex-1 );

        if ( sindex+1 >= info.size() )
            return false;
        db_info.dbname = info.substr( sindex+1, info.size()-sindex-1 );
    ...
    configuration::configuration() :
        daemon_(false), type_(server_type::ST_UNKNOWN), number_(0)
    {
    ...
    const database_info& configuration::get_db_info() const
    {
